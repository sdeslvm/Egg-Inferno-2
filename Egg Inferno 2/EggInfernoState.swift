...
protocol EggInfernoLoadStateRepresentable {
    var type: EggInfernoLoadState.StateType { get }
...
struct EggInfernoLoadState: Equatable, EggInfernoLoadStateRepresentable {
    ...
    static func idle() -> EggInfernoLoadState {
        EggInfernoLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> EggInfernoLoadState {
        EggInfernoLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> EggInfernoLoadState {
        EggInfernoLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> EggInfernoLoadState {
        EggInfernoLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> EggInfernoLoadState {
        EggInfernoLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: EggInfernoLoadState) -> Bool {
    ...
    static func == (lhs: EggInfernoLoadState, rhs: EggInfernoLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension EggInfernoLoadState: CustomStringConvertible {
